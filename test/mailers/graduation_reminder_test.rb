...
class GraduationReminderTest < ActionMailer::TestCase
  ...
  def test_notify_overdue_graduates_to_chief_instructor_role
    ...
    assert_mail_stored(2) { GraduationReminder.notify_overdue_graduates }

    mail = UserMessage.pending[0]
    assert_equal ['uwe@example.com'], mail.to
    assert_equal %w[noreply@test.jujutsu.no], mail.from
    assert_equal 'Disse medlemmene mangler gradering', mail.subject
    assert_match(/Voksne.*Lars Bråten.*5. kyu.*0.*4/m, mail.body)
  ...
  def test_notify_graduates
    now = Time.current
    censors(:uwe_voksne_upcoming).update! locked_at: now
    graduations(:voksne_upcoming).censors.create! member_id: id(:lars), examiner: true, locked_at: now
