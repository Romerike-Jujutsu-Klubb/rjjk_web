...
class RoleTest < ActiveSupport::TestCase
  test 'current appointments and elections change when role changed' do
    ...
    assert_equal 1, r.appointments(true).size
    assert_equal 1, r.elections(true).size

    r.update_attributes! years_on_the_board: 42

    assert_equal 0, r.appointments(true).size
    assert_equal 2, r.elections(true).size
    assert_equal 42, r.elections.includes(:annual_meeting).order('annual_meetings.start_at')[0].years
