...
class AttendanceHistoryGraph
  ...
  def self.month_chart(year, month, size)
    ...
    data, dates = month_chart_data(year, month)

    data.each do |group_name, values, group_color|
      g.dataxy(group_name, values, nil, group_color)
    end
  ...
  def self.month_per_year_chart(month, size)
    ...
    data, years = month_per_year_chart_data(month)

    data.each do |group, values, _color|
      g.dataxy(group, values.map { |v| [v['year'], v['count']] }, nil, Group.find_by(name: group).color)
    end
